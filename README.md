...
Yahoo Financeからトヨタ自動車（7203.T）の過去1年の日足株価を取得し、終値、20日移動平均線、50日移動平均線を確認できるStreamlitアプリです。現在は第2段階として、未来情報を含まない特徴量、翌営業日の方向を表す目的変数、時系列分割、LightGBMの学習・評価まで実装しています。モデル結果の画面表示は今後実装予定です。
...
バックテストと予測結果のStreamlit表示は今後実装予定です。
...
`app.py`（画面）から、`src/data_loader.py`（取得・整形）と`src/visualization.py`（グラフ）を呼び出します。学習処理は `src/features.py` が学習用DataFrameを作り、`src/model.py` が時系列分割、学習、予測、評価を担当します。第2段階ではまだUIから学習処理を呼び出しません。詳細は [docs/system_design.md](docs/system_design.md) を参照してください。
